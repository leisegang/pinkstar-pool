...
function Miner(id, login, workerName, pass, ip, port, startingDiff, noRetarget, pushMessage){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            miner = new Miner(minerId, login, workerName, params.pass, ip, port, difficulty, noRetarget, pushMessage);
